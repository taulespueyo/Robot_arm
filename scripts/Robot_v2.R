...
y_1 <- robot_df_dev["pos_dev_6"]
col_toSelect <- grep("TEMP|FORCE|VELOCITIES|\\.x\\.|\\.y\\.|\\.z\\.", names(robot_df_dev), value = TRUE)
x_1 <- robot_df_dev[col_toSelect]
...
y_2 <- robot_df_dev["pos_dev_6"]
test_index <- createDataPartition(robot_M2$ROBOT_TIME, times = 1, p = 0.8,  list = FALSE)
#test_robot2 <- robot_df2[-test_index, ]
#train_robot2 <- robot_df2[test_index, ] 

# Remove low-variance variables
low_var_indices <- nearZeroVar(robot_M2)
robot_filtered <- robot_M2[, -low_var_indices]
...
#robot_fil_noTarget <- robot_filtered_corr[, !names(robot_filtered_corr) %in% "pos_dev_6"]
target <- robot_df_dev["pos_dev_6"]
...
train_target <- target_scaled[test_index]
...
robot_rf_PCA <- foreach(ntree = rep(ntree / numCores, numCores), .combine = combine, .packages = 'randomForest') %dopar% {
  randomForest(robot_PCA$x[, 1:ncol(robot_PCA$x)], train_target, ntree = ntree, mtry = 5, nodesize = 20)
}
#reversed_robot_rf <- as.matrix(robot_rf_PCA) %*% t(robot_PCA$rotation[, 1:ncol(robot_PCA$rotation)])
acc_test <- postResample(pred = predict(robot_rf_PCA, train_pca2), obs = train_target)
acc_M2 <- postResample(pred = predict(robot_rf_PCA, test_pca2), obs = test_target)  # Accuracy 0.818 with 150 tres and 25 mtry; Accuracy 0.795 with 300 tres and 20 mtry (both overfitted to training set); Accuracy 0.769 with 150 trees and 10 mtry (overfitted to training set)
acc_test
acc_M2
...
x_1 <- robot_df1
robot_M3 <- data.frame(x_1, y_1)

test_index <- createDataPartition(robot_M3$pos_dev_6, times = 1, p = 0.8,  list = FALSE)
test_robot3 <- robot_M3[-test_index, ]
train_robot3 <- robot_M3[test_index, ] 
...
robot_rf <- randomForest(train_robot3$pos_dev_6 ~ ., data=train_robot3, ntree = 150, mtry = 25)
rmse_M3 <- sqrt(mean((unname(predict(robot_rf, test_robot3)) - test_robot3$pos_dev_6)^2))
acc_M3 <- postResample(pred = unname(predict(robot_rf, test_robot3)), obs = test_robot3$pos_dev_6) # Acc about 0.6, overfitted with 500 tres and 25 mtry; Acc 0.87 with 150 trees and 25 mtry
...
rf_tune_M3 <- tuneRF(
  train_robot3[, -1],    # Independent variables (excluding target variable 'mpg')
  train_robot3$pos_dev_6,      # Target variable
  stepFactor = 1.5,    # Step factor for increasing 'mtry'
  improve = 0.01,      # How much the improvement should be to stop tuning
  ntreeTry = 100,      # Number of trees to start with
  trace = TRUE         # Print progress of the tuning process
...
# # Make predictions on the test set with the best parameters
best_rf__M3 <- randomForest(
  pos_dev_6 ~ ., data = train_robot3,
  mtry = rf_tune_M3[1, 1],
  ntree = rf_tune_M3[1, 2]
)
